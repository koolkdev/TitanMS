...
int MySQL::connectToMySQL(){
	...
	if(!maple_db.connect("maplestory", "localhost", "root", "", 3306)){
...
int MySQL::showEquipsIn(int id, string equips[130][22]){
	Query query = maple_db.query();
	query << "SELECT * FROM equips WHERE charid=" << id;
	StoreQueryResult res = query.store();
	for(int i=0; i<(int)res.num_rows(); i++){
		for(int j=0; j<22; j++){
		equips[i][j] = string(res[i][j]);
		}
	}
	return (int)res.num_rows();
}
...
int MySQL::getItems(int id, int equips[400][4]){
	...
	for(int i=0; i<(int)res.num_rows(); i++){
		for(int j=0; j<5; j++){
			if(j>1)
				equips[i][j-1] = strval(string(res[i][j]));
			else if(j!=1)
				equips[i][j] = strval(string(res[i][0]));
		}
	}
	return (int)res.num_rows();
}
int MySQL::getSkills(int id, int skills[200][3]){
	Query query = maple_db.query();
	query << "SELECT * FROM skills WHERE charid=" << id;
	StoreQueryResult res = query.store();
	for(int i=0; i<(int)res.num_rows(); i++){
		for(int j=1; j<4; j++){
			skills[i][j-1] = strval(string(res[i][j]));
		}
	}
	return (int)res.num_rows();
}
int MySQL::getKeys(int id, int keys[90][3]){
	Query query = maple_db.query();
	query << "SELECT * FROM keymap WHERE charid=" << id;
	StoreQueryResult res = query.store();
	for(int i=0; i<(int)res.num_rows(); i++){
		for(int j=1; j<4; j++){
			keys[i][j-1] = strval(string(res[i][j]));
		}
	}
	return (int)res.num_rows();
}
void MySQL::getCharacter(int id, string info[28]){
	Query query = maple_db.query();
