...
public:
	PlayerNPC(int npcid, Player* player, bool isquest = 0, std::string sname = "");
	~PlayerNPC();
	...
	void addChar(char cha){
		char temp[2]={0};
		temp[0] = cha;
		addText(std::string(temp));
	}

	Packet npcPacket(char type);
	void setScriptName(std::string sname){
		this->sname = sname;
	}
	...
	void setGetText(std::string &text){
		gettext = text;
	}
	std::string getText(){
		return gettext;
