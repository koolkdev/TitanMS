...
private:
	...
	Channel* channel;
	PvP* pvp;
	stdext::hash_map <std::string, MapPortalData*> portals;
	stdext::hash_map <std::string, bool> portalsstatus;
public:
	...
	MapPortalData* getPortal(std::string name){
	...
	void showEffect(std::string& name);
	void showObject(std::string& name);
	...
	void setShip(bool s);
	bool getPortalStatus(std::string& name){
		if(portalsstatus.find(name) == portalsstatus.end())
			return false;
		return portalsstatus[name];
	}
	void enablePortal(std::string& name){
		if(portalsstatus.find(name) != portalsstatus.end())
