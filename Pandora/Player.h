...
//
	...
	void setMapPortal(int mapid, std::string& portalname);
	Map* getMap(){
		return this->map;
	}
	...
	Value addStat(int stat, bool random = false,int ammount = 1);
	...
	void setGlobalVariable(std::string &name, int val){
		global[name] = val;
	}
...
private:	
	...
	stdext::hash_map <std::string, int> global;
