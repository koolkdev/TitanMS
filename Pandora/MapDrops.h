...
private:
	Map* map;
	stdext::hash_map <int, Drop*> drops;
	bool timeron;
public:
	static int ids;
	MapDrops(Map* map){
	...
	bool findDrop(Drop* drop){
		for (stdext::hash_map<int,Drop*>::iterator iter = drops.begin(); iter != drops.end(); iter++)
			if(iter->second == drop)
				return true;
		return false;
	}
	void clear(){
		stdext::hash_map<int,Drop*> drops = stdext::hash_map<int,Drop*>(this->drops);
		for (stdext::hash_map<int,Drop*>::iterator iter = drops.begin(); iter != drops.end(); iter++)
