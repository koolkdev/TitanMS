...
public:
	...
	*//////////////////
	std::vector <Quest*> getQuestsInProgress(){
		std::vector<Quest*> ret;
		for (stdext::hash_map<int,Quest*>::iterator iter = pquests.begin(); iter != pquests.end(); iter++){
	...
	std::vector <Quest*> getCompletedQuests(){
		std::vector<Quest*> ret;
		for (stdext::hash_map<int,Quest*>::iterator iter = cquests.begin(); iter != cquests.end(); iter++){
			ret.push_back(iter->second);
		}
		return ret;
	}
...
private:
	Player* player;
	stdext::hash_map <int, Quest*> pquests;
	stdext::hash_map <int, Quest*> cquests;
};
