...
void PacketCreator::damage(Player *player, Damage& damage, int itemid){
	pw.writeInt(player->getID());
	pw.write(damage.getInfoByte());
	int skill = damage.getSkill();
	if(skill){
		pw.write(player->getSkills()->getSkillLevel(skill));
		pw.writeInt(skill);
	}
	else
		pw.write(0);
...
PacketWriter* PacketCreator::damageMob(Player *player, Damage& dmg){
...
PacketWriter* PacketCreator::damageMobMagic(Player *player, Damage& dmg){
...
PacketWriter* PacketCreator::damageMobRanged(Player *player, Damage& dmg, int itemid){
	pw.writeShort(DAMAGE_RANGED);

	damage(player, dmg, itemid);

	pw.writeShort(dmg.getPosition().x);
	pw.writeShort(dmg.getPosition().y);
