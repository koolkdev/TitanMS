...
void PacketCreator::itemInfo(Item* item, bool showPosition, bool showZeroPosition){
	...
	if(IS_PET(item->getID())){
		Pet* pet = (Pet*)item;
		pw.write(1); 
		pw.writeInt(((MapObject*)pet)->getID()); 
		pw.writeInt(0);
		pw.write(0);
		pw.writeBytes("8005BB46E61702"); // Cash?
		...
		pw.writeInt(0); // Or 0?
		pw.writeInt(0); // Expire time trial time
		return;
	}
	else if(cash){
		pw.write(0); 
	...
	if(item->getType() == EQUIP){
		...
		pw.writeShort(0); // Item flags?
		pw.write(0);
		pw.write(0); //Level
		pw.writeShort(0);
		pw.writeShort(0); //Item Exp
		pw.writeInt(0); //Hammer
		pw.writeLong(-1);
		pw.writeBytes("0040E0FD3B374F01");
		pw.writeInt(-1);
	} else{
...
PacketWriter* PacketCreator::showPetText(int playerid, int petSlot, std::string text, int act){
